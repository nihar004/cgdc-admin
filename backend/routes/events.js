...
routes.get("/batch/:batch_year", async (req, res) => {
  try {
    ...
    // Helper function to fetch position details
    const fetchPositionDetails = async (positionIds) => {
      ...
      const positionResult = await db.query(positionQuery, [positionIds]);

      return positionResult.rows.map((row) => ({
        id: row.id,
    ...
    const eventsPromises = result.rows.map(async (row) => {
      ...
      const positions = await fetchPositionDetails(row.position_ids);

      return {
        id: row.id,
        company_id: row.company_id,
        position_ids: row.position_ids || [],
        positions: positions, // Array of {id, title}
        title: row.title,
        ...
        roundNumber: row.is_placement_event ? row.round_number : null,
